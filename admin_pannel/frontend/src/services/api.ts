...
export const sosApi = {
    ...
    getEventById: async (id: string): Promise<SosEvent> => {
        const response = await apiClient.get<ApiResponse<SosEvent>>(`/api/sos-alerts/${id}`);
        return response.data.data!;
    },
    ...
    createEvent: async (event: {
        ...
        const response = await apiClient.post<SosEvent>('/api/sos-alerts', {
            ...event,
    ...
    updateStatus: async (id: string, status: 'acknowledged' | 'resolved'): Promise<SosEvent> => {
        const response = await apiClient.patch<ApiResponse<SosEvent>>(`/api/sos-alerts/${id}/status`, { status });
        return response.data.data!;
    },
};
